...
export default function App() {
  ...
  const handleAnalyze = async (mode: "A" | "B" | "C") => {
    ...
    try {
      ...
      console.error(e);
      setErrorMsg("品学长廊由于雾海阻隔暂时阻塞，已转至本地美学引擎进行即时分析。");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF8] text-[#2C2C2B] pb-20 selection:bg-[#8C927F]/20 selection:text-[#2C2C2B]" style={{ fontFamily: "'Georgia', 'Helvetica Neue', Arial, sans-serif" }}>
      ...
      <header className="max-w-7xl mx-auto px-6 pt-12 md:pt-16 pb-6 text-center overflow-visible">
        ...
        <p className="font-mono text-[9px] md:text-[10px] tracking-[0.3em] uppercase text-[#2C2C2B]/50 mt-4 font-light">
          Literary Aesthetics Model — AI Operation Manual
        </p>
        ...
        <div className="mt-6 border-t border-b border-[#2C2C2B]/10 py-3 flex flex-wrap justify-center gap-x-6 gap-y-1 text-xs font-serif italic text-[#2C2C2B]/75">
          <span className="not-italic font-sans text-[10px] font-semibold tracking-wider text-[#2C2C2B]/90 uppercase px-2 py-0.5 bg-[#8C927F]/10 rounded-sm">
            Autumn Book
          </span>
      ...
      {/* Exquisite Footer */}
      <footer className="mt-24 border-t border-[#2C2C2B]/10 pt-10 text-center text-xs text-[#2C2C2B]/40 font-serif max-w-4xl mx-auto flex flex-col items-center gap-4">
        <div className="flex gap-4 tracking-widest text-[9px] uppercase font-light">
          <span>Aesthetics Engine V2.5</span>
          <span>•</span>
          <span>Autumn Archive</span>
          <span>•</span>
          <span>Private Model Gateway Connected</span>
        </div>
        <p className="font-sans text-[10px] font-light leading-relaxed">
          文字审美模型 · 基于形式主义批评学传统
          <br />
          Copyright © 2026. Custom Crafted with True Dedication. All Rights Reserved.
        </p>
      </footer>
